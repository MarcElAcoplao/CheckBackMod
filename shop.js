...
if (x==24) {
  ...
  if (game.coins >= 250000 && pets[game.selectedPet][1] > pets[46][1] && upgrade24requirement() >= 23 && game.highestLevel >= 250000) {
    game.coins -= 250000
    alert("Bought " + items[x][0])
    itemChosen = x
   }
 else {alert("Not enough items; " + numberShort(game.coins) + "/250k coins, " + pets[game.selectedPet][1] + "/747 XP multi from pets, " + upgrade24requirement() + "/23 Upgrades maxed, " + numberShort(game.highestLevel) + "/250k Highest level")
 itemChosen = 0}
 }
}
if (x==25) {
...
if (x==35) {
  ...
  if (game.level >= 50000000 && game.items[29] >= 1 && game.pets[30] >= 5 && game.pets[38] >= 1 && game.pets[69] >= 1) {
    game.pets[30] -= 5
    game.pets[38] -= 1
    game.pets[69] -= 1
    alert("Bought " + items[x][0])
...
function updateShopBoosts() {
  ...
  game.itemXPBoost = (1 + (game.items[3] * 0.1) + (game.items[21] * 0.4)) * (99 * game.items[34] + 1) 
  ...
  game.itemXPBoostEffectSoftcap = 0 + (game.items[7] * 0.025) + (game.items[28] * game.tier * 2.5 ** Math.min(game.items[30], 1))/1000
