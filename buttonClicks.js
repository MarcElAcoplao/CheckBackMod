...
function clickButton(x) {
  if (game.buttonCooldowns[XPButtons[x].cooldownID] == 0) { //Checks the proper button is off cooldown
    ...
    game.XP += XPButtons[x].xp * (pets[game.selectedPet][1] * game.XPBoostEffect * game.itemXP * (1 + game.petsDiscovered / 100) * game.tierXPmulti * game.artifactsXP * game.loopMulti * game.oneSmithMulti ** game.oneSmithExpo) //Assigns the xp that you have to get
    if (game.items[43] == 0) {
...
function autoXPButtons() {
  if (game.loopUpgrades[1] >= 1) {
    clickAll()
  }
}
setInterval(autoXPButtons, 50)
...
function click2Button(x) { //Will work for any of them individually
  if (game.buttonCooldowns[XPBoostButtons[x].cooldownID] == 0) { //Checks if the button is ready and got enough XP
    game.XPBoost += XPBoostButtons[x].xpboost * pets[game.selectedPet][4] * game.itemXPBoost * game.artifactsXPBoost * game.loopMulti
...
function autoXPBButtons() {
  if (game.loopUpgrades[2] >= 1) {
...
function click3Button(x) {
  if (game.buttonCooldowns[StatButtons[x].cooldownID] == 0) {
    game.HP += 1 * StatButtons[x].stats * game.itemStat * game.tierStats * game.artifactsStats * game.loopMulti
    game.DMG += 0.1 * StatButtons[x].stats * game.itemStat * game.tierStats * game.artifactsStats * game.loopMulti
    game.DEF += 0.01 * StatButtons[x].stats * game.itemStat * game.tierStats * game.artifactsStats * game.loopMulti
...
function autoStatButtons() {
  if (game.loopUpgrades[3] >= 1) {
...
setInterval(autoStatButtons, 50)
...
function autoTokenButtons() {
  if (game.loopUpgrades[5] >= 1) {
...
setInterval(autoTokenButtons, 50)